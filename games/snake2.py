...
class Spike(Drawable):
    ...
    def __init__(self):
        super().__init__([32, 32], ["#ff0", "#ff0"], [random.randrange(0, 10) * 32, -32])

    def update(self):
        self.position[1] += 3
        AfterImage(self.size, self.colors, self.position.copy())
        super().update()

class AfterImage(Drawable):
    colorText = ["f", "e", "d", "c", "b", "a", "9", "8", "7", "6", "5", "4", "3", "2", "1", "0"]

    def __init__(self, size, color, position):
        super().__init__(size, color, position)
        game.afterImages.append(self)
    ...
    def update(self):
        ...
        for y in self.colors:
            for x in y:
                if x != "0" and x != "#":
                    tempCol[a] += self.colorText[self.colorText.index(x) + 1]
                else:
                    tempCol[a] += x
            a += 1
        self.colors = tempCol
...
class Game():
    ...
    def update(self):
        ...
        self.spikeSpawnDelay += 1

        while self.spikeSpawnDelay >= 60:
            self.spikeSpawnDelay -= 60
        ...
        y = 0
        for x in range(len(self.afterImages)):
            self.afterImages[y].update()
            if self.afterImages[y].colors[0] == "#000":
                canvas.delete(self.afterImages[y].rect)
                self.afterImages.pop(y)
                y -= 1
            y += 1
